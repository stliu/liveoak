...
public class AuthzServiceRootResourceTest extends AbstractResourceTestCase {
    ...
    @After
    public void after() {
        authzService.getAuthzService().setPolicies(null);
    }
    ...
    public void testAuthzServiceRequest() throws Exception {
        ...
        Assert.assertNull(policiesProperty);

        addPolicy(AuthzDecision.ACCEPT);

        reqCtx = new RequestContext.Builder().build();
        state = connector.read(reqCtx, "/authz");
        policiesProperty = (String) state.getProperty("policies");
        Assert.assertNotNull(policiesProperty);

        Assert.assertTrue(policiesProperty.contains("Mock Policy"));
    ...
    private void addPolicy(AuthzDecision decision) {
        ...
        authzService.getAuthzService().setPolicies(Collections.singletonList(policyEntry));
    }
